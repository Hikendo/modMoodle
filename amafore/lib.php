 ...
 * @return string
 */
function theme_amafore_get_main_scss_content($theme) {
    global $CFG;
    ...
    // amafore scss.
    $amaforevariables = file_get_contents($CFG->dirroot . '/theme/amafore/scss/amafore/_variables.scss');
    $amafore = file_get_contents($CFG->dirroot . '/theme/amafore/scss/default.scss');
    $security = file_get_contents($CFG->dirroot . '/theme/amafore/scss/amafore/_security.scss');

    $lastpreset = '';
    if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_amafore', 'preset', 0, '/', $filename))) {
        $lastpreset = $presetfile->get_content();
    }

    // Combine them together.
    $allscss = $amaforevariables . "\n" . $scss . "\n" . $amafore . "\n" . $lastpreset .    "\n" . $security;
 ...
 * @return string
 */
function theme_amafore_get_extra_scss($theme) {
    $content = '';
    ...
    if (empty($loginbgimgurl)) {
        $loginbgimgurl = new \moodle_url('/theme/amafore/pix/loginbg.png');
        $loginbgimgurl->out();
    }
 ...
 * @return string
 */
function theme_amafore_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
function theme_amafore_get_precompiled_css() {
    global $CFG;

    return file_get_contents($CFG->dirroot . '/theme/amafore/style/moodle.css');
}
 ...
 * @return mixed
 */
function theme_amafore_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    $theme = theme_config::load('amafore');
