 ...
 * @return string
 */
function theme_certify_get_main_scss_content($theme) {
    global $CFG;
    ...
    // certify scss.
    $certifyvariables = file_get_contents($CFG->dirroot . '/theme/certify/scss/certify/_variables.scss');
    $certify = file_get_contents($CFG->dirroot . '/theme/certify/scss/default.scss');
    $security = file_get_contents($CFG->dirroot . '/theme/certify/scss/certify/_security.scss');

    $lastpreset = '';
    if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_certify', 'preset', 0, '/', $filename))) {
        $lastpreset = $presetfile->get_content();
    }

    // Combine them together.
    $allscss = $certifyvariables . "\n" . $scss . "\n" . $certify . "\n" . $lastpreset .    "\n" . $security;
 ...
 * @return string
 */
function theme_certify_get_extra_scss($theme) {
    $content = '';
    ...
    if (empty($loginbgimgurl)) {
        $loginbgimgurl = new \moodle_url('/theme/certify/pix/loginbg.png');
        $loginbgimgurl->out();
    }
 ...
 * @return string
 */
function theme_certify_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
function theme_certify_get_precompiled_css() {
    global $CFG;

    return file_get_contents($CFG->dirroot . '/theme/certify/style/moodle.css');
}
 ...
 * @return mixed
 */
function theme_certify_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    $theme = theme_config::load('certify');
